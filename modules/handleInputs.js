...
export const handleInput = (event, object, objectValue, errorMessage) => {
  ...
  if(type === 'number') {
    ...
    output = setMin && (result < Number(setMin)) ? '' : setMax ? numberClamp(Number(result), setMin, setMax) : Number(result);
  } else {
    output = value;
  }
  ...
  return object ? { ...object, [name]: output } : { [name]: output };
}


// HANDLE INPUT ARRAY
export const handleInputArray = (index, data) => (e) => {
